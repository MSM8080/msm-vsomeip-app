...
void on_message(const std::shared_ptr<vsomeip::message>& _request) {
    ...
    std::cout << "SERVICE: Received message with Client/Session [" << std::setw(4) << std::setfill('0') << std::hex
              << _request->get_client() << "/" << std::setw(4) << std::setfill('0') << std::hex << _request->get_session() << "] "
              << ss.str() << std::endl;
...
int main() {
    ...
    /*********** app->offer_service() ************
     * we have to trigger the application to offer an instance of a service.
     * This can be done by adding an offer_service command
     */
